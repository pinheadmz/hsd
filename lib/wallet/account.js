...
class Account extends bio.Struct {
  ...
  async syncDepth(b, receive, change) {
    ...
    // Function input `receive` represents the minimum receiveDepth
    // we want to be at, given addresses that were just discovered in a TX.
    // If we haven't discovered the current receive address yet, we don't
    // need to derive any more new keys.
    if (receive > this.receiveDepth) {
      // The "real" depth of the wallet - how many keys are already in the DB.
      const depth = this.receiveDepth + this.lookahead;

      assert(receive <= depth + 1);

      // So we derive and save new keys, starting from
      // the current "real" depth and up to the desired "real" depth
      // which is the desired receiveDepth plus the lookahead.
      // If we discovered in a TX an address index higher than the current
      // receiveDepth (but of course still within the lookahead)
      // we will have to generate more than one key here to catch up.
    ...
    // Return all newly-derived receive addresses
    return result;
  }
