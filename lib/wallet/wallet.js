...
class Wallet extends EventEmitter {
  ...
  async syncOutputDepth(tx) {
    ...
    // Find the highest BIP44 index for both receive and change in this TX.
    for (const [acct, paths] of map) {
      ...
      // Account properties receiveDepth and changeDepth are 1-indexed,
      // meaning for brand new wallets, the receive address is at BIP44
      // index 0 but the receiveDepth is 1.
      // Account.syncDepth() is going to test if receive > receiveDepth
      // before deriving new addresses. If the address discovered in this
      // transaction is the current receive address, we want that to happen.
      // That's why we += 2 here:
      receive += 2;
      change += 2;
      ...
      const rings = await account.syncDepth(b, receive, change);

      if (rings.length > 0)
        derived = derived.concat(rings);
    }
