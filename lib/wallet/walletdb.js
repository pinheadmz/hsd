...
class WalletDB extends EventEmitter {
  ...
  _bind() {
    ...
    this.client.bind('block connect', async (entry, txs) => { console.log('block conenct', entry.height)
      try {
    ...
    this.client.hook('block rescan', async (entry, txs) => { console.log('block rescan', entry.height)
      try {
  ...
  async addBlock(entry, txs) {
    const unlock = await this.txLock.lock();
    try {
      console.log('add block', entry.height)
      return await this._addBlock(entry, txs);
    } catch (e) {
      if (e.type === 'LookaheadError') {
        this.logger.warning(
          'Account lookahead exceeded while adding block %x (%d)',
          entry.hash,
          entry.height
        );
        return await this._rescan(entry.height - 1);
      } else {
        throw e;
      }
    } finally {
  ...
  async _addBlock(entry, txs) {
    ...
    // Track total number of transactions added to wallets in the DB.
    let total = 0;

    // We want to track how many new addresses are derived by
    // accounts in wallets in the DB. If an account derives more
    // addresses than its own lookahead value, that means there
    // could be more transactions in the block for the wallet
    // that we didn't insert (because we didn't know the addresses
    // at the time that TX was being scanned).
    // Therefore, we consider it an Error if an account runs out of
    // lookahead addresses while scanning a single block. We must
    // update the Bloom Filter (for rescans, remote wallet node and SPV)
    // and then scan the block again. If a rescan is in progress, it is
    // aborted and then restarted after the update.
    const derivedByAccount = new Map();
    const handler = (wallet, derived) => {
      for (const addr of derived) {
        if (!derivedByAccount.has(wallet.wid))
          derivedByAccount.set(wallet.wid, new Map());

        const walletMap = derivedByAccount.get(wallet.wid);

        if (!walletMap.has(addr.account)) {
          walletMap.set(addr.account, 1);
        } else {
          walletMap.set(
            addr.account,
            walletMap.get(addr.account) + 1
          );
        }
      }
    };
    ...
    try {
      ...
      // Check for lookahead breaches
      for (const [wallet, walletmap] of derivedByAccount) {
        for (const [acct, derived] of walletmap) {
          const account = await this.getAccount(wallet, acct);
          if (derived >= account.lookahead)
  ...
  async rescanBlock(entry, txs) {
    ...
    try {
      ...
      if (e.type === 'LookaheadError') {
        this.logger.warning(
          'Account lookahead exceeded while rescanning block %x (%d)',
          entry.hash,
          entry.height
        );
        await this.abortRescan();
        // await this._rescan(entry.height - 1);
      } else {
        this.emit('error', e);
        throw e;
      }
  ...
  async _addTX(tx, block) {
    ...
    for (const wid of wids) {
      ...
      const {details} = await wallet.add(tx, block);
...
class LookaheadError extends Error {
  constructor() {
    ...
    this.type = 'LookaheadError';
    this.message = 'Wallet account lookahead exceeded';
    this.code = 'ERR_LOOKAHEAD';
  }
}
