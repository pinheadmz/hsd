...
class RootServer extends DNSServer {
  ...
  async response(req, rinfo) {
    ...
    if (name === '.') {
      ...
      // Signal answer is from HNS root zone
      this.setHandshakeEDNS(res, 1);
    ...
    if (!data) {
      ...
      if (item && item.root) {
        ...
        if (res.ad && res.code !== codes.NXDOMAIN) {
          ...
          // Signal answer is NOT from HNS root zone
          this.setHandshakeEDNS(res, 0);
  ...
  setHandshakeEDNS(res, hns) {
    if (!this.edns)
      return;

    assert((hns & 0xff) === hns);

    // Add the HANDSHAKE edns option to the response.
    // The server will determine if it was requested or not.
    // This way the data will always be cached.
    const opt = new HANDSHAKEOption();
    opt.hns = hns;
    res.edns.add(opt);
    res.edns.enabled = true;
  }
